...
def plot_all_figures_both(train_losses, test_losses,
    ...
    """
    pwd_saving = "/pbs/home/l/lperon/work_JUNO/figures/MLP/unique/"
    ...
    for i, (diff_train, diff_test, coord, unit) in enumerate(zip(
        ...
        plt.xlabel(fr'${coord}_{{true}} - {coord}_{{model}}$ [{unit}]', fontsize=14)
        plt.legend()
...
def plot_all_figures_energy(train_losses, test_losses,
    ...
    test_losses : list of float
        Test losses recorded over each epoch.
    avg_diff_E_train : list of float
        Lists of average differences between true and predicted values for energy (E) in training data.
    std_diff_E_train : list of float
        Lists of standard deviations of differences for E in training data.
    avg_diff_E_test : list of float
        Lists of average differences between true and predicted values for E in testing data.
    std_diff_E_test : list of float
        Lists of standard deviations of differences for E in testing data.
    diff_E_train : numpy.ndarray
        Arrays of differences between true and predicted values for E in training data.
    diff_E_test : numpy.ndarray
        Arrays of differences for E in testing data.
    batch_size : int, optional
        Batch size used during model training (default is 50).
    dropout_prop : float, optional
        Dropout probability used in model training (default is 0.2).
    ...
    - Histograms of prediction errors are provided for error analysis.
    """
    pwd_saving = "/pbs/home/l/lperon/work_JUNO/figures/MLP/double/energy/"
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(pwd_saving + f"unique_energy_diff_E_vs_epochs_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(pwd_saving + f"unique_energy_std_diff_E_vs_epochs_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
    ...
    fig.legend()
    fig.tight_layout()
    plt.savefig(pwd_saving + f"unique_energy_loss_long_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
    ...
    plt.xlabel(fr'$E_{{true}} - E_{{model}}$ [MeV]', fontsize=14)
    plt.ylabel('Density', fontsize=14)
    plt.legend(fontsize=12)
    plt.title('Energy Prediction Error', fontsize=16)
    plt.tight_layout()
    plt.savefig(pwd_saving + f"unique_energy_diff_long_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
...
def plot_all_figures_spatial(train_losses, test_losses,
    ...
    test_losses : list of float
        Test losses recorded over each epoch.
    avg_diff_x_train, avg_diff_y_train, avg_diff_z_train : list of float
        Lists of average differences between true and predicted values for coordinates (x, y, z) in training data.
    std_diff_x_train, std_diff_y_train, std_diff_z_train : list of float
        Lists of standard deviations of differences for x, y, z in training data.
    avg_diff_x_test, avg_diff_y_test, avg_diff_z_test : list of float
        Lists of average differences between true and predicted values for x, y, z in testing data.
    std_diff_x_test, std_diff_y_test, std_diff_z_test : list of float
        Lists of standard deviations of differences for x, y, z in testing data.
    diff_x_train, diff_y_train, diff_z_train : numpy.ndarray
        Arrays of differences between true and predicted values for x, y, z in training data.
    diff_x_test, diff_y_test, diff_z_test : numpy.ndarray
        Arrays of differences for x, y, z in testing data.
    batch_size : int, optional
        Batch size used during model training (default is 50).
    dropout_prop : float, optional
        Dropout probability used in model training (default is 0.2).
    ...
    - Histograms of prediction errors are provided for error analysis.
    """
    pwd_saving = "/pbs/home/l/lperon/work_JUNO/figures/MLP/double/spatial/"
    ...
    for i, (avg_train, avg_test, std_train, std_test, coord) in enumerate(zip(
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(pwd_saving + f"unique_spatial_diff_{coord}_vs_epochs_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(pwd_saving + f"unique_spatial_std_diff_{coord}_vs_epochs_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
    ...
    fig.legend()
    fig.tight_layout()
    plt.savefig(pwd_saving + f"unique_spatial_loss_long_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
    ...
    plt.tight_layout()
    plt.savefig(pwd_saving + f"unique_spatial_diff_long_lr={lr:.0e}_batch_size={batch_size:.0f}_dropout_prop={dropout_prop:.1f}.svg")
